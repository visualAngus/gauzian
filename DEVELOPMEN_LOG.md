...
### [2026-02-21] - fix(security): Durcissement backend auth/drive/agenda

**Contexte** : Passe de hardening sécurité backend Rust/Axum (OWASP) suite à revue ciblée auth, erreurs API, et téléchargement fichiers.

**Corrections appliquées** :
- ✅ **Suppression d'exposition de données sensibles** dans la réponse user info :
   - `password_hash` retiré
   - `auth_salt` retiré
- ✅ **Sanitization des erreurs internes** retournées au client :
   - plus de `DB error: ...`, `JWT error: ...`, `Redis error: ...` en payload API
   - messages génériques côté client + détails conservés dans les logs serveur
- ✅ **Protection contre l'injection d'en-tête** sur download :
   - nom de fichier nettoyé avant `Content-Disposition`
   - suppression caractères CR/LF, guillemets et antislash
- ✅ **Durcissement rate-limit login** :
   - normalisation email via `trim().to_ascii_lowercase()` pour éviter les contournements par casse/espaces

**Fichiers modifiés** :
- `src/auth/repo.rs`
- `src/auth/handlers.rs`
- `src/auth/services.rs`
- `src/agenda/handlers.rs`
- `src/drive/handlers.rs`

**Validation** :
- ✅ `cargo check` réussi (0 erreur)
- ✅ diagnostics éditeur : aucun problème sur les fichiers modifiés
